...
const deleteCategory = async (id: string) => {
    if (category.value?.products.length) {
        toast.add({
            title: 'Warning',
            description: 'This category has products. Please delete the products first.',
            timeout: 2000,
            color: 'yellow',
        });
        return;
    ...
    <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex justify-between items-center">
                        <div class="flex flex-col space-y-2">
                            ...
                            <div class="flex items-center space-x-2 mt-2" v-if="category.parent">
                                <UIcon name="fluent-emoji-flat:tag" class="w-5 h-5 text-gray-400" />
                                <span class="text-gray-400">
                                    Parent: {{ category.parent.name }}
                                    <ULink :to="`/warehouse/categories/${category.parent.id}`"
        ...
        <div v-if="category && category.products.length">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="product in category.products" :key="product.id" class="flex">
                    <UCard :ui="{ padding: 'p-4', background: 'bg-white dark:bg-gray-800' }"
                                ...
                                <div class="flex flex-col space-y-2">
                                    <div class="flex flex-col gap-4">
                                        ...
                                        <span
                                            ...
                                            ID:
                                            <ULink :to="`/warehouse/products/${product.id}`" active-class="text-primary-800"
                        ...
                        <div class="flex flex-col gap-4">
                            <div class="flex justify-between items-center">
                                <div class="flex flex-col space-y-2">
                                    ...
                                    <div class="flex items-center space-x-2 mt-2">
                                        <USkeleton class="h-5 w-full" :ui="{ rounded: 'rounded-full' }" />
                                        <USkeleton class="h-4 w-[100px]" />
                                    </div>
                                </div>
                                <div class="flex items-center">
