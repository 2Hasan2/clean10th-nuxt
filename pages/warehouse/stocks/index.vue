...
<template>
  <div>
    ...
    <UTable :loading="loading" :loading-state="{ icon: 'i-heroicons-arrow-path-20-solid', label: 'Loading...' }"
      :progress="{ color: 'primary', animation: 'carousel' }" :rows="products" :columns="columns">
      <template #name-data="{ row }">
        <ULink :to="`/warehouse/products/${row.id}`"
      ...
      <template #actions-data="{ row }" v-if="$user.role === 'ADMIN'">
        <UButtonGroup>
