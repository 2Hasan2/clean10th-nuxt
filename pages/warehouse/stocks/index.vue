...
<template>
  <div>
    ...
    >
        ...
        <ULink
          ...
          :class="
            row.stock?.quantity > 0
              ? 'text-green-500 dark:text-green-400 hover:text-green-700 dark:hover:text-green-200'
      ...
      <template #actions-data="{ row }" v-if="$user.value?.role === 'ADMIN'">
        <UButtonGroup>
          <UButton
            color="blue"
            @click="navigateTo(`/warehouse/stocks/${row.stock?.id}`)"
          >
            Edit
          </UButton>
        </UButtonGroup>
